...
class LocationService {
  ...
  async findNearbyLocations(
    ...
    const locations = await db
      ...
      .select([
        ...
        'updated_at',
        sql<string>`ST_AsText(location)`.as('location'),
        sql<number>`ST_Distance(location, ST_SetSRID(ST_MakePoint(${longitude}, ${latitude}), 4326)::geography)`.as('distance')
      ])
      .where(
  ...
  // Add a new location
  async addLocation(name: string, latitude: number, longitude: number): Promise<void> {
    await db
  ...
  async getDistanceBetweenLocations(id1: number, id2: number): Promise<number | null> {
    const result = await db
      .selectFrom('locations as l1')
      .innerJoin('locations as l2', sql`true`)
      .select(sql<number>`ST_Distance(l1.location::geography, l2.location::geography)`.as('distance'))
      .where('l1.id', '=', id1)
      .where('l2.id', '=', id2)
      .executeTakeFirst();
...
async function main(): Promise<void> {
  try {
    ...
    allLocations.forEach(location => {
      console.log(`  ${location.name}: (${location.location.latitude}, ${location.location.longitude})`);
    });

    // Find locations near Times Square (within 2km)
    console.log('\n🔍 Locations near Times Square (within 2km):');
    const nearbyLocations = await locationService.findNearbyLocations(40.7580, -73.9855, 2000);
    ...
    if (allLocations.length >= 2) {
      ...
      console.log(`\n📏 Distance between ${allLocations[0].name} and ${allLocations[1].name}: ${distance ? Math.round(distance) : 'unknown'} meters`);
    }
